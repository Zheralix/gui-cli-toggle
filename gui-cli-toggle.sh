...
case "${1}" in
        ...
        "")
                ...
                echo "Then you must enter the correct password for every iteration of the loop."
                ...
                if [[ "${gui_cli_toggle_variable_TARGET}" == "graphical.target" ]]; then
                        ...
                        if [[ -n "${gui_cli_toggle_variable_DM}" ]]; then
                                echo "Disabling display maanger: ${gui_cli_toggle_variable_DM}"
        ...
        -h|--help)
                echo "gui-cli-toggle version ${gui_cli_toggle_variable_VERSION}"
                echo ""
                echo "gui-cli-toggle [-h --help] [-v --version]"
